...
- **`heal-automated-tests`** — triages failing specs (test defect / isolation defect / intent change / product bug / product fixed), fixes only what belongs to the test, and escalates the rest — case updates via `maintain-test-cases`, defects into QA Vault plus the project's tracker.
