...
2. **Search first (dedupe).** Before authoring, search existing cases for overlap — semantic **and** title, since either alone misses (see the `search-test-cases` modes). Phrase the semantic query as a full description of the behaviour, not two or three keywords; short queries find their target only about half the time. Then **confirm each candidate with `get_test_case` before calling it a near-duplicate** — the server's `relevance` tier is a starting point, and a `related` hit may share nothing but vocabulary. Surface the confirmed overlaps and let the engineer choose: new / update existing / skip. An empty search result means nothing cleared the relevance floor — it is not proof the area is uncovered.
