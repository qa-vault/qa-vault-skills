...
2. **Find affected cases.** Search **broadly** — a change usually ripples past the obvious area (a new 2FA login step also touches password-reset, remember-me, and any case asserting "reaches the dashboard"). Use semantic **and** title search to gather candidates (see the `search-test-cases` modes), phrasing each semantic query as a full description of the behaviour rather than a couple of keywords. Then `get_test_case` each candidate and **judge affectedness from its actual `steps` and descriptive fields (`description`, `preconditions`) — not its title.** A title like "User logs in" won't mention the changed step, so a title-only scan silently drops cases whose steps do reference it. Never edit **or dismiss** a case you haven't read in full.

   Breadth here is a completeness claim, so treat the response's own accounting as part of the search: a listing that reports a `total_count` above what it returned is one page of several — page it out with `offset` before concluding. Don't cap a semantic search with `limit`; the server already returns everything it judged relevant, and a cap can only hide matches. A missed case here doesn't fail loudly — it ships as an approved expected result that no longer matches the product.
...
- **Update / extend** stale cases. Rewrite the affected `steps`/fields per case with `update_test_case` — edits differ case to case, and this is the only tool that can change steps at all. `bulk_update_test_cases` takes **no `steps` and no `tags`**; it applies one identical value for a scalar field (priority, status, layer, suite…) to every id you give it, so reserve it for exactly that. Note that `update_test_case`'s `tags` **replaces** a case's whole tag set rather than adding to it — send the full intended list, or you will silently strip the others. When an edit changes **what the case verifies**, regenerate its `search_abstract` in the same call (format and rules in `create-test-cases`) — the abstract is the case's second face in semantic search, and one describing the old behavior keeps matching queries the case no longer answers. If maintenance reveals **mass** abstract absence across a project (many cases with no `search_abstract` at all), that is not this skill's job — hand it to `backfill-search-abstracts`.
